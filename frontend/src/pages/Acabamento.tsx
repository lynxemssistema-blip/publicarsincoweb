...
import {
 Plus, Search, Edit2, Trash2, X, Paintbrush, Save,
 Loader2, RefreshCw
} from 'lucide-react';
...
interface Acabamento {
 IDAcabamento?: number;
 DescAcabamento: string;
}

const emptyForm: Acabamento = {
 DescAcabamento: ''
};
...
export default function AcabamentoPage({ isModal = false, onCloseModal }: Props = {}) {
 const [acabamentos, setAcabamentos] = useState<Acabamento[]>([]);
 const [formData, setFormData] = useState<Acabamento>(emptyForm);
 ...
 a.DescAcabamento?.toLowerCase().includes(searchTerm.toLowerCase())
 );
 ...
 try {
 const url = isEditing ? `${API_BASE}/acabamento/${formData.IDAcabamento}` : `${API_BASE}/acabamento`;
 ...
 try {
 const res = await fetch(`${API_BASE}/acabamento/${id}`);
 const json = await res.json();
 if (json.success) {
 ...
 if (!confirm('Deseja realmente excluir este acabamento?')) return;
            ...
            <div className="flex items-center justify-between p-5 border-b border-gray-100">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-lg font-semibold text-[#32423D]">
                  {isEditing ? 'Editar Acabamento' : 'Novo Acabamento'}
                </h2>
              </div>
              <button
                onClick={resetForm}
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-5">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">
                  Descrição Acabamento <span className="text-red-500 font-bold">*</span>
                </label>
                <input
                  type="text"
                  name="DescAcabamento"
                  value={formData.DescAcabamento || ''}
                  onChange={handleInputChange}
                  placeholder="Digite a descrição do acabamento..."
                  className={inputRequired}
                  maxLength={200}
                  required
                />
                <p className="text-xs text-gray-400 mt-1">Máximo 200 caracteres</p>
              </div>
 ...
 <p className="text-gray-500 text-xs">Gerencie o cadastro de tipos de acabamento</p>
 </div>
 <div className="flex gap-2">
 <motion.button
 whileHover={{ scale: 1.02 }}
 whileTap={{ scale: 0.98 }}
 onClick={fetchAcabamentos}
 ...
 <th className="px-2 py-0.5 text-left text-xs font-semibold text-white uppercase tracking-wider">Descrição Acabamento</th>
 ...
 <p className="text-xs">Nenhum acabamento encontrado</p>
 <button
 onClick={() => setShowForm(true)}
 className="text-[#32423D] font-medium text-xs hover:underline"
 >
 Cadastrar novo acabamento
 </button>
 </div>
 </td>
 </tr>
 ) : (
 filteredAcabamentos.map((acabamento, idx) => (
 <motion.tr
 key={acabamento.IDAcabamento}
 initial={{ opacity: 0, y: 10 }}
 animate={{ opacity: 1, y: 0 }}
 ...
 <button
 onClick={() => acabamento.IDAcabamento && handleEdit(acabamento.IDAcabamento)}
 ...
 <button
 onClick={() => acabamento.IDAcabamento && handleDelete(acabamento.IDAcabamento)}
 ...
 Mostrando <span className="font-medium">{filteredAcabamentos.length}</span> de <span className="font-medium">{acabamentos.length}</span> acabamentos
 </p>
 </div>
 )}
 </div>
