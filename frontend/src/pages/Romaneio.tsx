...
import {
 ...
 List, PlusSquare, Box, AlertTriangle, MessageSquare, Printer, Edit3
} from 'lucide-react';
...
export default function RomaneioPage() {
 ...
 const [reportItems, setReportItems] = useState<Record<string, unknown>[]>([]);
 ...
 const [motoristas, setMotoristas] = useState<Record<string, unknown>[]>([]);
 ...
 const [tiposTransporte, setTiposTransporte] = useState<Record<string, unknown>[]>([]);
 ...
 const [availableItems, setAvailableItems] = useState<Record<string, unknown>[]>([]);
 ...
 const [insertedItems, setInsertedItems] = useState<Record<string, unknown>[]>([]);
 ...
 const handleIncludeItem = async (item: Record<string, unknown>, qty: string) => {
 ...
 ? (jsonCheck.data || []).find((r: Record<string, unknown>) => r.IdOrdemServicoItem === item.IdOrdemServicoItem)
 : null;
 ...
 try {
 const params = new URLSearchParams(itemFilters as Record<string, unknown>);
 ...
 try {
 const params = new URLSearchParams(insertedFilters as Record<string, unknown>);
 ...
 const _gv = (obj: Record<string, unknown>, key: string) => {
 if (!obj) return '';
 const fk = Object.keys(obj).find(k => k.toLowerCase() === key.toLowerCase());
 return fk ? String((obj as Record<string, unknown>)[fk] || '').trim().toUpperCase() : '';
 };
 ...
 const r = romaneio as Record<string, unknown>;
 ...
 {/* List */}
 <div className="flex-1 flex flex-col min-h-0 bg-white rounded-md shadow-sm border border-gray-100 overflow-hidden mt-3">
 ...
 initial={{ opacity: 0, scale: 0.95 }}
 animate={{ opacity: 1, scale: 1 }}
 className="bg-white rounded-md shadow-xl w-full max-w-6xl h-[92vh] flex flex-col overflow-hidden"
 >
 ...
 {/* Toolbar */}
 <div className="flex flex-wrap items-center justify-between gap-4 bg-white p-4 rounded-md shadow-sm border border-gray-100 print:hidden">
 ...
 {/* Report Content */}
 <div className="bg-white rounded-md shadow-md border border-gray-100 overflow-hidden print:shadow-none print:border-none">
 {/* Header Info */}
