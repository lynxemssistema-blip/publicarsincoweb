...
export default function MotoristaPage() {
 ...
 {motorista.ImagemCNH ? (
 <div className="relative inline-block group/img" onMouseLeave={() => setZoomLevel(1)}>
 <img src={getAuthImageUrl(motorista.ImagemCNH)} alt="CNH" className="mx-auto w-8 h-8 rounded-full object-cover border border-gray-200 cursor-zoom-in shadow-sm relative" />
 
 {/* Fullscreen Lightbox View on Hover */}
 <div className="fixed inset-0 z-[100] flex items-center justify-center opacity-0 group-hover/img:opacity-100 transition-opacity duration-300 pointer-events-none group-hover/img:pointer-events-auto bg-black/40 backdrop-blur-sm" onWheel={handleWheel}>
 <img src={getAuthImageUrl(motorista.ImagemCNH)} alt="CNH Ampliada" className="max-w-[90vw] max-h-[90vh] object-contain drop-shadow-2xl rounded-lg transition-transform duration-75" style={{ transform: `scale(${zoomLevel})` }} />
 </div>
 </div>
 ) : (
 <span className="text-gray-300">-</span>
