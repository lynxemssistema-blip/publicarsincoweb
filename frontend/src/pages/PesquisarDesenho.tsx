...
export default function PesquisarDesenho() {
 ...
 if (valor && strVal !== '') return 'bg-[#E0E800]/30 text-[#32423D] border-blue-200';
 ...
 <button onClick={fetchDesenhos} disabled={loading} className="px-4 py-1.5 flex items-center justify-center gap-2 bg-emerald-100 border border-emerald-200 text-emerald-800 rounded shadow-sm text-[11px] font-bold hover:bg-emerald-200 transition-colors disabled:opacity-50 flex-1 lg:flex-none">
 ...
 {/* Grid */}
 <div className="flex-1 bg-white border border-slate-200 rounded-md overflow-hidden flex flex-col min-h-0 shadow-sm relative">
 ...
 {items.map(item => (
 <tr key={`osi-${item.IdOrdemServicoItem}`} className="hover:bg-[#E0E800]/10 transition-colors">
 <td className="px-2 py-1 bg-white sticky left-0 z-10 shadow-[1px_0_0_rgba(0,0,0,0.05)] flex gap-1 items-center">
 {/* 1 - Abrir Desenho 3D */}
 <button onClick={() => abrirArquivoLocal(item.EnderecoArquivo, '3d')} className="p-1 text-slate-500 hover:text-[#32423D] hover:bg-[#E0E800]/10 rounded" title="1 - Abrir Desenho 3D">
 <Box size={14} />
 </button>
 {/* 2 - Abrir Desenho PDF */}
 ...
 ? <span className="px-1.5 py-0.5 bg-[#E0E800]/40 text-[#32423D] rounded text-[9px] font-bold">SIM</span> 
 : <span className="px-1.5 py-0.5 bg-slate-100 text-slate-500 rounded text-[9px] font-bold">NÃO</span>}
 </td>
