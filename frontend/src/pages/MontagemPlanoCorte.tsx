...
function PainelItensPlano({ plano, onFechar, aglutinado, setAglutinado, onGerarRnc, onItemRemoved }: { 
 ...
 onGerarRnc: (item: Record<string, unknown>) => void;
 onItemRemoved?: () => void;
}) {
 const { token } = useAuth();
...
// ============================================================
function PainelPlanosCorte({ refreshTrigger, externalOnGerarRnc }: { refreshTrigger: number; externalOnGerarRnc?: (item: Record<string, unknown>) => void }) {
 const { token } = useAuth();
 const { addToast } = useToast();
 ...
 const [itemSelecionado, setItemSelecionado] = useState<unknown>(null);
 ...
 const [tipostarefa, setTipostarefa] = useState<unknown[]>([]);
 ...
 const handleGerarRnc = (item: Record<string, unknown>) => {
 ...
 <div className="flex items-center gap-3">
 <div className="p-2.5 bg-primary/10 rounded-md text-primary shadow-sm">
 <Scissors size={24} />
 </div>
 <div>
...
function ModalRnc({ 
 ...
 rncForm: RncFormData; setRncForm: Record<string, unknown>; isSaving: boolean; msg: Record<string, unknown>; 
 setActionModal: Record<string, unknown>; salvarNovaRnc: Record<string, unknown>; usuarios: unknown[]; tipostarefa: unknown[]; 
 ...
 return (
 <div className="fixed inset-0 bg-slate-900/60 z-[100] flex items-center justify-center p-3 animate-in fade-in duration-300">
 <div className="bg-white rounded-md shadow-2xl w-full max-w-4xl border border-slate-200 flex flex-col" style={{ maxHeight: '95vh' }}>
 {/* ── Cabeçalho ── */}
 ...
 onChange={e => setRncForm((p: Record<string, unknown>) => ({...p, titulo: e.target.value.toUpperCase()}))}
 ...
 onChange={e => setRncForm((p: Record<string, unknown>) => ({...p, subTitulo: e.target.value.toUpperCase()}))}
 ...
 onChange={e => setRncForm((p: Record<string, unknown>) => ({...p, descricao: e.target.value.toUpperCase()}))}
 ...
 onChange={e => setRncForm((p: Record<string, unknown>) => ({...p, [proc.key]: e.target.checked}))} />
 ...
 <select value={rncForm.setor} onChange={e => setRncForm((p: Record<string, unknown>) => ({...p, setor: e.target.value}))} className={selectCls}>
 ...
 <select value={rncForm.usuario} onChange={e => setRncForm((p: Record<string, unknown>) => ({...p, usuario: e.target.value}))} className={selectCls}>
 <option value="">Selecione...</option>
 {usuarios.map(u => <option key={u.IdUsuario ?? u.id ?? u.NomeCompleto} value={u.NomeCompleto ?? u.label}>{u.NomeCompleto ?? u.label}</option>)}
 </select>
 </div>
 ...
 <select value={rncForm.tipoTarefa} onChange={e => setRncForm((p: Record<string, unknown>) => ({...p, tipoTarefa: e.target.value}))} className={selectCls}>
 <option value="">Selecione...</option>
 ...
 onChange={e => setRncForm((p: Record<string, unknown>) => ({...p, dataExec: e.target.value}))}
 className={inputCls} />
 </div>
 </div>
 </div>
 ...
 className="px-5 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-md shadow-md shadow-orange-500/30 transition-all disabled:opacity-50 flex items-center gap-1.5 font-black text-[11px]">
...
export default function MontagemPlanoCortePage() {
 ...
 const [pageItemSelecionado, setPageItemSelecionado] = useState<unknown>(null);
 const [pageIsSaving, setPageIsSaving] = useState(false);
 const [pageMsg, setPageMsg] = useState<{ t: 's' | 'e'; m: string } | null>(null);
 const [pageUsuarios, setPageUsuarios] = useState<unknown[]>([]);
 const [pageTipostarefa, setPageTipostarefa] = useState<unknown[]>([]);
 ...
 const handlePageGerarRnc = (item: Record<string, unknown>) => {
 ...
 <div className="flex items-center gap-3">
 <div className="h-10 w-10 bg-blue-100 text-[#32423D] rounded-md flex items-center justify-center shadow-sm shrink-0"><Scissors size={22} /></div>
 <div>
