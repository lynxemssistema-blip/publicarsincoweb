...
const startString = "app.get('/api/acompanhamento/projetos', tenantMiddleware, async (req, res) => {";
...
// Inside the route, find the query
const queryStartStr = "const [rows] = await req.tenantDbPool.execute(`";
const queryStartIdx = content.indexOf(queryStartStr, startIdx);

if (queryStartIdx === -1) {
    console.error("Could not find query start!");
    process.exit(1);
}

// Find the end of the query
const queryEndStr = "LIMIT 300\n        `);";
const queryEndIdx = content.indexOf(queryEndStr, queryStartIdx);
...
const beforeQuery = content.substring(0, queryStartIdx);
const afterQuery = content.substring(queryEndIdx + queryEndStr.length);

const optimizedQuery = `const [rows] = await req.tenantDbPool.execute(\`
            SELECT
...
const finalContent = beforeQuery + optimizedQuery + afterQuery;
fs.writeFileSync(filePath, finalContent, 'utf8');
console.log('Patch applied strictly to /api/acompanhamento/projetos!');
