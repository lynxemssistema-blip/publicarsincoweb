...
const target = `res.status(500).json({ success: false, message: 'Erro interno ao buscar recursos' });
    }
});`;
...
app.get('/api/ordemservico/:idTag/tempos-producao', tenantMiddleware, async (req, res) => {
    try {
        ...
        try {
            const query = 'SELECT ?? AS Setup, ?? AS Padrao, ?? AS Total FROM ordemservico WHERE IdTag = ? LIMIT 1';
...
if (!content.includes('/api/ordemservico/:idTag/tempos-producao')) {
    content = content.replace(target, target + newEndpoint);
