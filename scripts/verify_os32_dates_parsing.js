...
async function verifyOS32() {
  ...
  console.log('--- TEST RESULTS FOR OS 32 DATES ---');
  sectors.forEach(sec => {
    const { pi, pf } = getSectorPlanningDates(os, sec);
    const isoPi = toIsoInput(pi);
    const isoPf = toIsoInput(pf);
    console.log(`Recurso: ${sec}`);
    console.log(`  Raw in DB: pi="${pi}", pf="${pf}"`);
    console.log(`  Formatted for <input type="date">: pi="${isoPi}", pf="${isoPf}"`);
  });
  process.exit(0);
}
